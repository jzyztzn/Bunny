...
deepspeed bunny/train/train.py \
    --lora_enable True --lora_r 256 --lora_alpha 256 --mm_projector_lr 1e-5 \
    --deepspeed ./script/deepspeed/zero3.json \
    --model_name_or_path /path/to/meta-llama/Meta-Llama-3-8B-Instruct \
    --model_type $MODEL_TYPE \
    --version llama \
    --data_path ./data/finetune/bunny_allava_1.3m.json \
...
* Lastly, linearly avearge two models
  ...
  for i in range(1, total + 1):
      ...
      for k in model_1.keys():
          avg[k] = model_1[k] * 0.42 + model_2[k] * 0.58 # the weight factor is selected empirically
